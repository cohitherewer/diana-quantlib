...
class DigitalQuantOp(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, x: torch.Tensor, div, zero: torch.Tensor, clip_hi: torch.Tensor
    ):
        ctx.save_for_backward(x)
        quantized_x = x / div
        return torch.clip(
            torch.floor(quantized_x), min=zero, max=clip_hi
        )  # clipped
