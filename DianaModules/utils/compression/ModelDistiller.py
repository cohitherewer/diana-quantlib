...
class DistilLoss(nn.Module):
    ...
    def forward(self, distill_outputs, target):
        ...
        l_kl = (
            nnf.kl_div(p, q, reduction="sum")
            * (self.T**2)
            / student_logits.shape[0]
        )
        l_ce = nnf.cross_entropy(student_logits, target)
        return l_kl * self.alpha + l_ce * (1.0 - self.alpha)
...
class QModelDistiller(pl.LightningModule):
    ...
    def optimizer_step(
        ...
        if self.trainer.global_step < self.hparams.warm_up:
            lr_scale = min(
                1.0, float(self.trainer.global_step + 1) / self.hparams.warm_up
            )
    ...
    def configure_optimizers(self):
        try:
            ...
            raise RuntimeError(
                f"Unknown optimizer type {self.hparams.optimizer}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"Unknown optimizer type {self.hparams.lr_scheduler}"
            )
        ...
        if opt_type is OptimizerType.SGD:
            ...
            raise RuntimeError(
                f"Unsupported Optimizer type: {self.hparams.optimizer}"
            )

        if sched_type is LrScheduler.COSINE:
            schedulers = [
                CosineAnnealingLR(
                    opts[-1], T_max=self.hparams.max_epochs, eta_min=0.0
                )
            ]
        elif sched_type is LrScheduler.MULTISTEP:
            # decay lr at 30%, 60% and 80%
            milestones = [
                int(r * self.hparams.max_epochs) for r in (0.3, 0.6, 0.8)
            ]
            schedulers = [
                MultiStepLR(
                    opts[-1], milestones=milestones, gamma=self.hparams.gamma
                )
            ]
