...
VALID_PATTERNS = [
    [AnalogAccumulator, DigitalRequantizer],
    [AnalogAccumulator, AnalogRequantizer],
    [AnalogAccumulator, MulAdd, "add", EpsTunnel, DigitalRequantizer],
    [AnalogAccumulator, "add", EpsTunnel, DigitalRequantizer],
]
...
class SIMDnodesFinder(Finder):
            ...
            try:
                if isinstance(g.get_submodule(node.target), AnalogAccumulator):
                    ...
                    for i in range(4):  # max depth 4
                        ...
                        current_node = user
                        if (
                            user.op in FXOpcodeClasses.CALL_METHOD.value
                            and "add" in user.target
                        ):
                            pattern.append(DianaAps("add", user))
                            continue
                        elif (
                            user.op in FXOpcodeClasses.CALL_FUNCTION.value
                            and "add" in user.target.__name__
                        ):
                            pattern.append(DianaAps("add", user))
                            continue
                        ...
                        if isinstance(
                            module, (DigitalRequantizer, AnalogRequantizer)
                        ):
                            pattern.append(DianaAps("Requantizer", user))
                            break
                        else:  #  Mul Add  , Res Add , Activation
                            pattern.append(
                                DianaAps(str(type(module).__name__), user)
                            )
                    assert (
                        len(pattern) <= 5
                    )  # max lenght is Accumulator , muladd modul , add , requantizer so 3
                    aps.append(pattern)
    ...
    def check_aps_commutativity(self, aps: List[List[DianaAps]]) -> bool:
        return len(aps) == len(
            set(ap.node for lap in aps for ap in lap)
        )  # each `fx.Node` should appear at most once
...
class SIMDVerifier:
    ...
    def verify(self, input: torch.Tensor) -> None:
        ...
        for pattern in self.aps:
            self.SIMDparameters.append(
                self.extract_features_from_pattern(pattern)
            )
        ...
        for idx, node in enumerate(self.model_input_output.keys()):
            ...
            SIMD_out = SIMDModel.simd_op(
                self.SIMDparameters[idx],
                self.model_input_output[node]["input"].numpy(),
                resadd,
            )
    ...
    def isvalid_pattern(
        self, pattern: List[DianaAps]
    ) -> bool:  # this is redundant but good for
        is_valid = True
    ...
    def extract_features_from_pattern(
        self, pattern: List[DianaAps]
    ) -> SIMDLayer:
        layer = SIMDLayer()
        for ap in pattern:
            if ap.type == "Requantizer":
                ...
                layer.bitwidth_clip = torch.round(
                    torch.log2(module.clip_hi - module.clip_lo)
                ).item()
                ...
                ap.node.op in FXOpcodeClasses.CALL_METHOD.value
                or ap.node.op in FXOpcodeClasses.CALL_FUNCTION.value
            ):
                # extract res_in r_s (EpsTunnel)
                res_in_module: EpsTunnel = self.model.get_submodule(
                    next(
                        iter(
                            filter(
                                lambda node: isinstance(
                                    self.model.get_submodule(node.target),
                                    EpsTunnel,
                                ),
                                ap.node.all_input_nodes,
                            )
                        )
                    ).target
                )
                layer.r_s = torch.log2(
                    self.model_input_output[pattern[0].node.target][
                        "resadd_scale"
                    ]
                ).item()
                layer.res_in = (
                    self.model_input_output[pattern[0].node.target]["resadd"]
                    / self.model_input_output[pattern[0].node.target][
                        "resadd_scale"
                    ]
                ).numpy()
                ...
                module = self.model.get_submodule(ap.node.target)
                layer.q_s += torch.round(
                    torch.log2(module.eps_out / module.eps_in)
                ).item()
    ...
    def register_hooks(self):
        for l in self.aps:
            ...
            accumulator_module.register_forward_hook(
                self.register_out_hook(l[0].node.target, "input")
            )
            activation_module = self.model.get_submodule(
                l[len(l) - 1].node.target
            )
            activation_module.register_forward_hook(
                self.register_out_hook(l[0].node.target, "output")
            )
            # res add
            for i in range(len(l)):
                ...
                node = ap.node
                if (
                    node.op in FXOpcodeClasses.CALL_METHOD.value
                    or node.op in FXOpcodeClasses.CALL_FUNCTION.value
                ):  # addition
                    input_nodes = [p for p in node.all_input_nodes]
                    for input in input_nodes:
                        if isinstance(
                            self.model.get_submodule(input.target), EpsTunnel
                        ):
                            resadd_module = self.model.get_submodule(
                                input.target
                            )
                            break
                    resadd_module.register_forward_hook(
                        self.register_out_hook(l[0].node.target, "resadd")
                    )
                    self.model_input_output[l[0].node.target][
                        "resadd_scale"
                    ] = (resadd_module.eps_out / resadd_module.eps_in)
                    break
