        ...
        for n in onnxproto.graph.node:
            ...
            if op_type in dory_onnxnode_op_types['linear']:
                ...
                if isinstance(pytorch_module, (nn.Linear, nn.Conv1d, nn.Conv2d, nn.Conv3d)):
                    weight_bits = 8 if  pytorch_module.is_analog == torch.Tensor([1]) else 3# check register buffer inside conv layer to know if it's analog or not   
                    bias_bits = 32  
                    ...
                    if isinstance(pytorch_module, nn.Conv2d): 
                        annotations.append(onnx.helper.make_attribute(key='weight_data_layout', value ='OIHW'))
                        annotations.append(onnx.helper.make_attribute(key='input_data_layout' , value = 'NCHW')) 
...
class DianaExporter(DORYExporter) : 
    ...
    def export(self, network: nn.Module, input_shape: torch.Size, path: os.PathLike, name: Optional[str] = None, opset_version: int = 10) -> None:
        ...
        data_folder = Path("backend/optimized/optimized_model_NO_ANNOTATION.onnx")
        ...
        self._annotator.annotate(network, str(data_folder.absolute()))
