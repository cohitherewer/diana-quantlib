...
from quantlib.algorithms.qbase.qhparamsinitstrategy.qhparamsinitstrategy import ConstInitStrategy, MinMaxInitStrategy
from .Descriptor import ModuleDescriptor
from DianaModules.core.Operations import DianaBaseOperation , DIANAConv2d , DIANALinear , AnalogConv2d
        ...
        for node in fq_model.graph.nodes: 
            try: 
                ...
                if isinstance(module , DianaBaseOperation) : 
                    if isinstance(module , (DIANAConv2d, DIANALinear)) : # for now depth-wise sep not supported 
                        core_choice = 'DIGITAL' if type(module) != AnalogConv2d else 'ANALOG' #TODO When depthwise seperable convolutions are added , implement the feature here 
                        if type(module._qinitstrategy)  == ConstInitStrategy: 
                            qhparamsinitstrategyspec = {'CONST' : module.default_kwargs}
