...
class AnalogFeatureExtractor:  # Linear layers followed by identities
    def __init__(self, model: nn.Module) -> None:
        ...
        self.model = model
        self.graph_mod = qg.fx.quantlib_symbolic_trace(
            root=model
        )  # graph module
                ...
                if module.is_analog == torch.Tensor([True]):
                    ...
                    while True:
                        ...
                        if isinstance(user_mod, Requantisation):
                            break
                    # print(user_mod)
                    self.model_input_output.append(
                        {
                            node.target: {
                                "scale": user_mod.div,
                    ...
                    pred_mod.register_forward_hook(
                        self._out_hook(
                            node.target,
                            len(self.model_input_output) - 1,
                            "input",
                        )
                    )
                    user_mod.register_forward_hook(
                        self._out_hook(
                            node.target,
                            len(self.model_input_output) - 1,
                            "output",
