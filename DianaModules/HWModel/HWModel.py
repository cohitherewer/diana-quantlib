...
class SIMDModel:
    ...
    def _bn(cls, layer, in_fmap, row=0):  # batch norm
        ...
        for c in range(in_fmap.shape[1]):
            out_fmap[row][c] = (
                layer.bn_w[c][row] * (2**layer.bn_ws)
            ) * in_fmap[row][c] + (layer.bn_b[c] * 2**layer.bn_bs)
        return out_fmap
    ...
    def _clip(cls, in_fmap, bw):
        ...
        out_fmap = (
            (in_fmap * clip_min_mask_n * clip_max_mask_n)
            + min_filler
            + max_filler
        )
        return out_fmap
    ...
    def simd_op(
        ...
        layer,
        in_fmap0: np.ndarray,
        in_fmap1: Union[np.ndarray, None] = None,
    ):  # forward pass
            ...
            for channel in range(in_fmap.shape[1]):
                for row in range(in_fmap.shape[2]):
                    for c in range(in_fmap.shape[3]):
                        out_fmap[batch][channel][row][c] = (
                            layer.bn_w[0][channel][0][0].item()
                            * (2**layer.bn_ws)
                        ) * in_fmap[batch][channel][row][c].item() + (
                            layer.bn_b[0][channel][0][0].item()
                            * 2**layer.bn_bs
                        )
        return out_fmap
