...
model.eval()
data_loader = {'train': ut.DataLoader(train_dataset, batch_size=128, shuffle=True, pin_memory=True) , 'validate' : ut.DataLoader(validation_dataset, batch_size=256, shuffle=True  ,pin_memory=True)}
...
out_path = output_weights_path + "/"+'ResNet_Mixedweights.pth'
optimizer = torch.optim.Adam(_Mixed_model.gmodule.parameters(), lr = 0.04 ,  weight_decay=1e-5)


params =  DianaModule.train(_Mixed_model.gmodule,optimizer,data_loader, epochs=1, model_save_path=out_path ) 
