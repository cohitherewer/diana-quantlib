...
print("STARTING FQ")
resnet18_diana = DianaModule(DianaModule.from_trainedfp_model(fp_model, modules_descriptors=module_descriptions) ) 
print("FINISHED FQ")
...
#resnet18_diana.load_state_dict(torch.load(model_save_path)["state_dict"]) 
##torch.save ({
...
##training 
#checkpoint_callback = ModelCheckpoint(monitor = "val_acc" , dirpath='zoo/imagenet/resnet18/FQ/',filename='imgnet-{epoch:02d}-{val_acc:.4f}' ,save_top_k=1)
#callbacks = [checkpoint_callback ,EarlyStopping(monitor="val_acc", mode="max") ]
#max_epochs = 60 
...
#max_epochs = 30 # train the bound for 10 epochs
#checkpoint_callback = ModelCheckpoint(monitor = "val_acc" , dirpath='zoo/imagenet/resnet18/FQ_bounds/',filename='imgnet-{epoch:02d}-{val_acc:.4f}' ,save_top_k=1)
#callbacks = [checkpoint_callback ,EarlyStopping(monitor="val_acc", mode="max") ]
...
#momentum = 0.4
#resnet18_diana.set_optimizer('SGD' , lr=lr , momentum=momentum, weight_decay=weight_decay)
#checkpoint_callback = ModelCheckpoint(monitor = "val_acc" , dirpath='zoo/imagenet/resnet18/HW_mapped/',filename='imgnet-{epoch:02d}-{val_acc:.4f}' ,save_top_k=1)
#callbacks = [checkpoint_callback ,EarlyStopping(monitor="val_acc", mode="max") ]
#max_epochs = 50
