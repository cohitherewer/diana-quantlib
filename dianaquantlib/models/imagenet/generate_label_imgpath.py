...
imagenet_base_dir = os.path.normpath(
    str(Path("../../../../../projectdata/datasets/imagenet/small").absolute())
)
...
# load classes file
classes_labels = (
    pd.read_csv(classes_file, delimiter=" ", header=None, index_col=0)
    .iloc[:, :1]
    .to_dict()[1]
)
        ...
        for subdir in subdirs:
            for _, _, files in os.walk(path + f"/{subdir}"):
                for name in files:
                    ...
                    f.write(f"{subdir}/{name} {classes_labels[subdir]}\n")
