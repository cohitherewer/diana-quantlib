...
from dianaquantlib.core.Operations import AnalogConv2d
from dianaquantlib.utils.BaseModules import DianaModule
from quantlib.algorithms.qbase.qhparams.qhparams import (
    create_qhparams,
    get_scale,
)
from quantlib.algorithms.qbase.qrange.qrange import (
    QRangeSpecType,
    resolve_qrangespec,
)
...
class QuantDownStepper:
    def __init__(
        ...
        module: DianaModule,
        steps: int,
        initial_quant: QRangeSpecType,
        target_quant: QRangeSpecType,
        module_type=AnalogConv2d,
    ) -> None:
        ...
        self.n_levels_step = torch.ceil(
            (
                torch.round(torch.log2(initial_n_levels))
                - torch.round(torch.log2(self.target_n_levels))
            )
            / steps
        )
        ...
        if initial_n_levels <= self.target_n_levels:
            raise RuntimeError(
                "The QuantDownStepper module can only be used to step down quantization. Not step up!"
            )
        ...
        assert (
            initial_n_levels >= steps >= 1
        ), "Please decrease the steps and also ensure the step count is more than or equal to 1. Else just use regular quantization"

    def step(self) -> None:
        if self.current_step != 0:
            print(self.current_step)
            self.current_step -= 1
            for _, module in list(
                filter(
                    lambda m: True
                    if isinstance(m[1], self.module_type)
                    else False,
    ...
    def step_module(self, module: _QModule):
        ...
        n_levels = torch.round(
            torch.exp2(torch.log2(module.n_levels) - self.n_levels_step)
        )
        ...
        if module.n_levels <= 3:
            module.bw_clip_lo = torch.tile(
                torch.Tensor([-1]), module.clip_lo.shape
            )
            module.bw_clip_hi = torch.tile(
                torch.Tensor([1]), module.clip_hi.shape
            )
        else:
